## SIMULATE AGE-POLICIES BY AGE-GROUPS
...
""" Script to retrieve AGE-POLICIES for Italy, Spain, Germany, France

Specifically, there are simulated experiments which combine two possible strategies: 
-- Salutary Sheltering: targeted intervention for a specific fraction of an age groups which is completely isolated from the rest
-- Physical Distancing: population-level strategy which decreases the number of out-of-household contacts by a given factor 

(1) **AGEPOLICIES - SCENARIO 1** --> YES Salutary Sheltering for some age groups, NO Physical Distancing
Modeled by setting two conditions: a) NO Lockdown (Locdown date at 2030) b) YES Salutary Sheltering (factor for the proportion of those sheltered equal to 50%, 100% for age-group)
In this case the salutary sheltering is implemented to specific age groups

(2) **AGEPOLICIES - SCENARIO 2** --> YES Salutary Sheltering for some age groups, YES Physical Distancing 
Modeled by setting two conditions: a) YES Lockdown (Lockdown data at 2020) and a factor of 2 b) YES Salutary Sheltering (factor equal to 50%, 75%, 100% for age-group) 
In this case the salutary sheltering is targeted to each specific age groups

OUTCOME
In the end the simulated experiments are 6 (three for each scenario; 3x2), for each country and age-group: 
    (1) 50% Sheltering + No Distance - Scenario 1
    (2) 75% Sheltering + No Distance - Scenario 1
    (3) 100% Sheltering + No Distance - Scenario 1
    (4) 50% Sheltering + Yes Distance - Scenario 2
    (5) 75% Sheltering + Yes Distance - Scenario 2
    (6) 100% Sheltering + Yes Distance - Scenario 2
For each experiment, there are saved 12 Files in the relative Folders;
...
def simulate_policies_agepolicies(all_possible_combos):
    # Initialization
    ...
    mtti_val = all_possible_combos[0] # mean time to isolation value 
    combo_frac_stay_home = all_possible_combos[1] #combo fraction stay at home
    country = all_possible_combos[2] # country 
    val = all_possible_combos[3] # value of the stay at home 
    age_range_name = all_possible_combos[4] # name for the age_range of the tested policy
    distance_bool = all_possible_combos[5] # boolean indicating physical distancing or not  
    
    ### SET PARAMETERS 
    # Note: distance_bool is the argument determining whether there is (or there is not) physical distancing
    if country== 'Italy':
        real_pop_size = 60000000 # 60Millions 
        if distance_bool == True: 
            d_lockdown = date(2020, 3, 8) # lockdown >> YES LOCKDOWN (implies Physical Distancing)
        ...
        if distance_bool == True: 
            d_lockdown = date(2020, 3, 14) # lockdown >> YES LOCKDOWN (implies Physical Distancing)
        elif distance_bool == False: 
            d_lockdown = date(2030, 3, 14) # lockdown >> NO LOCKDOWN (implies NO Physical Distancing)
        d_end = date(2020, 5, 15) # stop
        d_stay_home_start = date(2020, 3, 14) 
        factor = 2.0
    elif country == 'Germany':
        real_pop_size = 80000000 # 80Millions
        if distance_bool == True: 
            d_lockdown = date(2020, 3, 13)  # lockdown >> YES LOCKDOWN (implies Physical Distancing)
        elif distance_bool == False: 
            d_lockdown = date(2030, 3, 13)  # lockdown >> NO LOCKDOWN (implies NO Physical Distancing)
        d_end = date(2020, 5, 15) # stop
        d_stay_home_start = date(2020, 3, 13) 
        factor = 2.0
    elif country == 'France':
        real_pop_size = 66000000 # 66Millions
        if distance_bool == True: 
            d_lockdown = date(2020, 3, 16)  # lockdown >> YES LOCKDOWN (implies Physical Distancing)
        elif distance_bool == False: 
            d_lockdown = date(2030, 3, 16)  # lockdown >> NO LOCKDOWN (implies NO Physical Distancing)
        d_end = date(2020, 5, 15) # stop
        d_stay_home_start = date(2020, 3, 16) 
        factor = 2.0
    
    EXP_ROOT_DIR = "age_policies"
    if distance_bool == True: 
        sim_name = '%s_yesdistance' %country ## Default simulation name
    elif distance_bool == False: 
        sim_name = '%s_nodistance' %country ## Default simulation name
    ...
    # Parse real data for computing mse
    ...
    ** Table: Hospitalization, intensive care unit (ICU) admission, and case–fatality percentages for reported COVID–19 cases, by age group —United States, February 12–March 16, 2020
    ...
    if distance_bool == True: 
        path = os.path.join(EXP_ROOT_DIR, sim_name)
        fname = '%s_AgePolicy_%s_%s_n%s_p%s_m%s_s%s'%(country,val,age_range_name,params['n'],pigc_val,mm_val, sd_val)
        fname += '_%s.csv'
    ...
    print(f"AGE-POLICIES {country}: VAL:{val}, AGE-RANGE:{age_range_name}, DISTANCE:{distance_bool}, D0:{sd_val}, PINF: {pigc_val}, DMULT:{mm_val}")
    print('################')
...
# ISOLATION POLICIES: Keep a certain percentage (50%,75%,100%) of some age group isolated
for a in age_ranges:
...
for country in country_list:
    ...
    for distance in [True,False]:
        for combo in all_possible_combos:
            combo = list(combo)
            lower = str(combo[1].nonzero()[0][0])
            upper = str(combo[1].nonzero()[-1][-1])
            val = str(combo[1][int(lower)])
    ...
    Parallel(n_jobs=15)(delayed(simulate_policies_agepolicies)(i) for i in tot_combos_list)